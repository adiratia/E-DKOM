...
static void find_rightmost(void) {
		...
		printk(KERN_INFO " ----------------------------------------------\n");
		printk(KERN_INFO "number of runnable processes: %d",task_list->se.cfs_rq->rq->nr_running);
		...
		if(rightmost!=NULL){
			for_each_process(rightmost_task) {
					if ( &rightmost_task->se.run_node == rightmost) {
						printk("right most task: : %s ; pid: %d\n", rightmost_task->comm, rightmost_task->pid);
						printk(KERN_INFO " ----------------------------------------------\n");
						//task_list->se.vruntime = rightmost_task->se.vruntime+1000000;
